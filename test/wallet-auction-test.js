...
describe('Wallet Auction', function() {
  ...
  describe('Duplicate Pending Requests', function() {
    ...
    it('should REGISTER and fail duplicate register', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Register is already pending: ${name1}, can't send yet.`);
    ...
    it('should UPDATE and fail duplicate UPDATE', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Update is already pending: ${name1}, can't send yet.`);
    ...
    it('should RENEW and fail duplicate RENEW', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Renewal is already pending: ${name1}, can't send yet.`);
    ...
    it('should TRANSFER and fail duplicate TRANSFER', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Transfer is already pending: ${name1}, can't send yet.`);
    ...
    it('should FINALIZE and fail duplicate FINALIZE', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Finalize is already pending: ${name1}, can't send yet.`);
    ...
    it('should REVOKE and fail duplicate REVOKE', async () => {
      ...
      assert(err);
      assert.strictEqual(err.message, `Revoke is already pending: ${name2}, can't send yet.`);
