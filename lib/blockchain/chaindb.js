...
const ChainMigrations = require('./migrations');
...
class ChainDB {
  ...
  async open() {
    const migrations = new ChainMigrations({
      ...this.options,
      chainDB: this,
  ...
  writeVersion(b) {
    const value = Buffer.alloc(this.dbName.length + 4);

    value.write(this.dbName, 0, 'ascii');
    value.writeUInt32LE(this.dbVersion, this.dbName.length);
  ...
  async verifyVersion() {
    ...
    if (data.length !== this.dbName.length + 4)
      throw new Error(error);

    if (data.toString('ascii', 0, this.dbName.length) !== this.dbName)
      throw new Error(error);

    const num = data.readUInt32LE(this.dbName.length);

    if (num !== this.dbVersion)
      throw new Error(error);
  }
