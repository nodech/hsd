...
class ChainDB {
  ...
  async open() {
    const chainDBVersion = 1;

    const migrations = new ChainMigrations({
      ...this.options,
      chainDB: this,
      dbVersion: chainDBVersion
    });
    ...
    if (!version) {
      // Database is fresh.
      // Write initial state.
      await this.initialize(chainDBVersion);
      this.logger.info('ChainDB successfully initialized.');
    } else {
      await this.verifyVersion(chainDBVersion);
   ...
   * @param {Number} version - chaindb version
   * @return {Promise}
   */
  ...
  async initialize(version) {
    this.start();
    try {
      await this._initialize(version);
    } catch (e) {
      this.drop();
      throw e;
    }
  ...
  async _initialize(version) {
    const b = this.batch();
    this.writeVersion(b, version);
    this.writeFlags(b);
