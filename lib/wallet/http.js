  ...
  initRouter() {
    ...
    this.post('/wallet/:id/send', async (req, res) => {
      ...
      const options = TransactionOptions.fromValidator(valid, this.network,
        res.abortController);
      TXCreateTimeoutAndRegister(req.wallet, options);
...
class TransactionOptions {
  ...
  constructor(valid, network, controller) {
    this.rate = null;
    this.maxFee = null;
    this.selection = null;
    ...
    if (valid)
      this.fromValidator(valid, network, controller);
  }
...
function TXCreateTimeoutAndRegister(wallet, options) {
  assert(wallet);

  if (feat.FEAT_ABORT_ON_BLOCK && options.controller) {
    if (!options.controller.signal.aborted)
      wallet.registerController(options.controller);
  }
...
HTTP.TXCreateTimeoutAndRegister = TXCreateTimeoutAndRegister;
